...
| arXiv ID | text | stable dedupe key without version suffix |
| 标题 | text | paper title |
| 作者 | text | author list |
| 机构 | text | confirmed affiliations |
| 发布时间 | datetime | arXiv published date |
| 更新时间 | datetime | arXiv updated date |
| 论文链接 | url or text | arXiv abstract URL |
| PDF 链接 | url or text | PDF URL |
...
| 去重键 | text | normally same as arXiv ID |
...
1. Fetch candidate arXiv IDs.
2. Check duplicate state:
   - Base: scan/query `arXiv ID` or `去重键`.
...
6. Mark the same arXiv IDs as seen with `scripts/arxiv_candidates.py mark-seen`.
...
lark-cli base +record-upsert \
  --base-token app_xxx \
  --table-id tbl_xxx \
  --json '{"arXiv ID":"2601.12345","标题":"...","是否已推送":false,"推送状态":"未推送"}'
```
...
今日检查了 <n> 篇候选论文，未发现符合分类、机构和作者背景筛选标准的新论文。
```
